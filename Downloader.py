...
class Downloader:
    ...
    def __init__(self, url=None, number_of_threads=1, destination_directory='', source_sha1sum=''):
        ...
        self.target_filename = os.path.join(destination_directory, os.path.basename(self.url))  # name of a file to be downloaded
        ...
        self.source_sha1sum = source_sha1sum
    ...
    def merge_chunks(self):
        print("Merging chunks into a single file ...")
        with open(self.target_filename, "ab") as target_file:
            for i in range(self.number_of_threads):
                print("Writing part {} of {}".format(i, self.number_of_threads-1))
    ...
    def get_downloaded_crc32c_large(self):
        prev = 0
        for eachLine in open(self.target_filename, "rb"):
            prev = zlib.crc32(eachLine, prev)
        return "%X" % (prev & 0xFFFFFFFF)

    def get_downloaded_sha1_large(self):
        BUF_SIZE = 65536
        sha1 = hashlib.sha1()
        ...
        if self.source_sha1sum:
            print('Source SHA1:', self.source_sha1sum)
            print('Equal SHA1:', self.source_sha1sum == downloaded_sha1)
        return sha1.hexdigest()

    def check_integrity(self):
        self.get_downloaded_crc32c_large()
